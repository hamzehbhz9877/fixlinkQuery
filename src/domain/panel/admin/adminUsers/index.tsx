...
const AdminList = () => {
    ...
    const {
        data,
        loadingMessage
    } = useGetQuery<adminUser[]>(getAllAdminUser, {
        ...
        <>
            ...
            <section >
                <button className="btn custom-mutate-button"
                        onClick={() => handleOpen(<AddAminUser close={handleClose}/>, {style:{content: {width: "600px"}}})}>
                    افزودن کاربر جدید
                </button>
                <Table tableHeadingItem={["نام کاربری", "نام و نام خانوادگی", "تعداد لینک ها", "تاریخ", "وضعیت", "حذف"]}>
                    {

                        data?.map(({
                                       isActive, countLink, userName, email, fullName, id, createAtPersian
                                   }, index: number) =>
                            <tr key={index}>
                                <td>{userName}</td>
                                <td>{fullName}</td>
                                ...
                                <td>
                                    <button
                                            onClick={() => handleOpen(<DeleteAminUser close={handleClose}
                                                                                      fullName={fullName}
                                                                                      id={id}/>)}>
