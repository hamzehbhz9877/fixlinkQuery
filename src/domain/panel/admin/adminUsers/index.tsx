...
const AdminList = () => {
  const { data, loadingMessage } = useGetQuery<adminUser[]>({
    event: getAllAdminUser,
    eventOption: {
  ...
  const toggle = useMutationQuery({ event:deActiveAdminUser });
    ...
    <>
      ...
      <section>
        ...
        >
          {data?.map(
            (
              { isActive, countLink, userName, fullName, id, createAtPersian},
              index: number
            ) => (
                <tr key={index}>
                    <td>{userName}</td>
