...
import {initialValues, validationSchema} from "domain/panel/admin/notification/validation";
...
const AddNotificationsList: FC<Props> = ({close,search}) => {
    ...
    const create = useMutationQuery<notifications["notifications"][0], notificationPost>(createNotification, "",
        {
            onError: () => {
                console.log("error")
            }
    ...
    const handleSubmit = async (values: notificationPost, actions: FormikHelpers<notificationPost>) => {
        ...
        create.restQuery.mutate(values, {
            onSuccess: (data: any) => {
                queryClient.setQueryData<notifications>(["notifications", 1, search],
                    ((old: any) => ({
                        ...old, notifications: [...old.notifications, data]
                    })));
                close()
            }
        })
