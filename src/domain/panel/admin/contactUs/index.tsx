...
const ContactUsPanel = () => {
  ...
  const { data, loadingMessage } = GetContactUs({
  ...
  return (
    <>
      <section className="dashboard-table">
        <SearchBar
          description={
            'در این قسمت شما میتوانید جزییات دقیق پیام های قرار داده شده را مشاهده کنید'
          }
          label={'کاربر'}
          setSearch={sendData}
          title={'جدول پیام ها'}
        ...
        >
          {data?.contactUs.map(
            (
              { createAtPersian, id, fullName, email, userName, text, subject },
              index: number
            ) => (
              <tr key={index}>
                <td>{userName}</td>
