...
const Dashboard = () => {
    ...
    const {data, loadingMessage} = GetAllLink({page: rest.currentPage, search: searchValue})
    ...
    return (
        <>
            <section >
                <SearchBar description={"در این قسمت شما میتوانید جزییات دقیق لینک های قرار داده شده را مشاهده کنید"}
                           label={"لینک"}
                           setSearch={sendData}
                           title={"جدول لینک ها"} url/>

                <Table tableHeadingItem={["لینک اصلی", "لینک کوتاه", "تعداد بازدید", "تاریخ ساخت", "وضعیت"
                    , "حذف"]} classes={"table__fixed"}
                       cols={<colgroup><col width="30%"/><col width="270px"/>
                ...
                >
                    {
                        data?.links.map(({
                            ...
                            <tr key={index}>
                                ...
                                <td><span>{referenceCount}</span></td>
                                ...
                                <td>
                                    <button
                                            onClick={() => handleOpen(<DeleteLink close={handleClose}
