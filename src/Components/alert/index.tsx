...
const Alert = ({id, message, type, remove, timeout, showProgress}: alert & { remove: (id: number) => void }) => {
    ...
    useEffect(() => {
        if (ref.current) {
            ...
            animationDuration.current = parseInt(getComputedStyle(ref.current).animationDuration)
        }
    ...
    return (
        <div className={`alert--show ${state && "alert--hide"}`} onClick={handleDelete}>
