...
const SearchBar: FC<Props> = ({
                                  title, description, setSearch, label,
                                  url,
                                  userPanelQrCode
                              }) => {
    ...
    const validationSchema = Yup.object({
        search: (url) ?
            Yup.string().url("لینک وارد شده نامعتبر میباشد").required('لطفا لینک کوتاه خود را را وارد کنید').trim()
                                    ...
                                    <>
                                        <div className="d-flex search__filter-wrapper">
                                            <div className="form__div">
                                                {
                                                    meta.value &&
                                                    <ClearSearch clearSearch={setSearch} reset={handleReset}/>
                                                }
                                                <input type={url ? "url" : "text"}
                                                       className={`form__input search__padding-btn ${url && "s-left"}
                                                            ${meta.error && meta.touched ? "error" : ""} 
                                                            ${meta.touched && !meta.error ? "success" : ""}`}
                                                       placeholder=" " {...field} />
