...
const UserList = () => {
    ...
    const {data, loadingMessage} = GetAllUser({page: rest.currentPage, search: searchValue})
    ...
    return (
        <>
            <section>
                <SearchBar description={"در این قسمت شما میتوانید جزییات دقیق کاربر های قرار داده شده را مشاهده کنید"}
                           label={"کاربر"}
                           setSearch={sendData}
                           title={"جدول کاربر ها"}/>
                <div className="table-overflow">
                    <Table
                        tableHeadingItem={["نام کاربری", "نام و نام خانوادگی", "ایمیل", "تعداد لینک ها", "تاریخ","بررسی وضعیت لینک ها", "وضعیت", "حذف", "ویرایش رمز"]}>
                        {
                        data?.userResponseDto.map(({
                            ...
                            <tr key={index}>
                                ...
                                <td><label className="switch">
                                    <input id={id} onClick={()=>handleDeActive(id)}
                                                                     type="checkbox" defaultChecked={isActive}/><span
                                    className="switch__slider switch--round"/></label></td>
                                <td>
                                    <button onClick={() => handleOpen(<DeleteUserList search={searchValue}
                                                                                      currentPage={rest.currentPage}
                                                                                      close={handleClose} id={id}
                                                                                      fullName={fullName}/>)}>
                                        <FaTrash className="fa-trash"/></button>
                                </td>
                                <td>
                                    <button onClick={() => handleOpen(<EditUserList search={searchValue}
                                                                                    currentPage={rest.currentPage}
                                                                                    close={handleClose}/>)}>
