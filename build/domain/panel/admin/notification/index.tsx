...
const Post = () => {
    ...
    const {data, loadingMessage} = GetNotification({page:rest.currentPage, search:searchValue})
    ...
    return (
        <>
            <section >
                <SearchBar description={"در این قسمت شما میتوانید جزییات دقیق اطلاعیه های قرار داده شده را مشاهده کنید"}
                           label={"اطلاعیه"}
                           setSearch={sendData}
                           title={"جدول اطلاعیه ها"}/>
                <button className="btn custom-mutate-button"
                        onClick={() => handleOpen(<AddNotificationsList search={searchValue} close={handleClose}/>, {
                ...
                <Table tableHeadingItem={["عنوان", "تاریخ", "حذف", "ویرایش"]}>
                    {
                        data?.notifications.map(({
                            ...
                            <tr key={index}>
                                ...
                                <td>
                                    <button
                                            onClick={() => handleOpen(<DeleteNotificationList
                                                search={searchValue}
                                ...
                                <td>
                                    <button onClick={() => handleOpen(<EditNotificationList search={searchValue}
                                                currentPage={rest.currentPage} close={handleClose}
                                                data={{text, title}} id={id}
                                            />,{style:{content: {width: "mex-content"}}})}>
                ...
                <Modal options={options} showModal={isModalOpen} close={handleClose} children={modalData}/>
            </section>
        </>
    );
}
