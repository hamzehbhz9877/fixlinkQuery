...
import {getDailyData, getMonthlyData, getWeeklyData, getYearlyData} from "Services/shortlink";
...
const LinkState = () => {
    ...
    const {restQuery: dailyQ, loadingMessage: l1} = useGetQuery<dailyData>(getDailyData,
        {
            queryKey: ["daily", data],
    ...
    const {restQuery: weeklyQ, loadingMessage: l2} = useGetQuery<weeklyData>(getWeeklyData,
        {
    ...
    const {restQuery: monthlyQ, loadingMessage: l3} = useGetQuery<monthlyData>(getMonthlyData,
        {
    ...
    const {restQuery: yearlyQ, loadingMessage: l4} = useGetQuery<yearlyData>(getYearlyData,
        {
            queryKey: ["yearly", data], loadingMessage: "درحال دریافت آمار سالیانه..."
        }, data, {
            enabled: false, onSuccess: (data) => {
                setInfo(data)
    ...
    return (
        <>
            ...
            <section className="card dashboard__chart text-center">
                {
                    (l1 || l2 || l3 || l4) ?? (info && info.length > 0 ?
                        <BarChart data={chart.data} chartOptions={{options:chart.options}} />
