...
const Login = () => {
    ...
    const handleSubmit = async (values: userPost, actions: FormikHelpers<userPost>) => {
        actions.resetForm();
        login.restQuery.mutate(values, {
            onSuccess: (data:any) => {
                ...
                if (data) {
                    console.log(data)
                    const expire = new Date(data.expires_in * 1000 + new Date().getTime())
                    cookie.set("token", data.jwt, {expires: expire,path:"/"});
                    cookie.set("user", data.userName, {expires: expire,path:"/"});
                    if (data.role) cookie.set("Ro", data.role, {expires: expire,path:"/"})
                    history.replace("/");
                }
            },
    ...
    return (
        <section className="login center-content">
            <div className="card login--card custom--card">
                    ...
                    {() => (
                        <Form>
                            ...
                            <div className="text-center">
                                <button type="submit" className="btn custom-btn">ورود</button>
                            </div>
                        </Form>
                    )}
                </Formik>
