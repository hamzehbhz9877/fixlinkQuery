...
const Main = () => {
    ...
    const {restQuery} = useMutationQuery<generatorLink, generateLinkPost>(generateLink)

    const [show, setShow] = useState(false)

    const handleSubmit = async (values: generateLinkPost, actions: FormikHelpers<generateLinkPost>) => {
        actions.resetForm();
        ...
        <section className="home center-content">
            <div className="home__generate-link">
                ...
                <Formik initialValues={initialValues} onSubmit={handleSubmit} validationSchema={validationSchema}>
                    {() => (
                        <Form>
                            <WInput name="bigLink" type="url" label={"کوتاه کن"}
                                    placeholder="https://FixLink.ir/sample"/>
                            {show && <Field name="shortLink">
                            ...
                            {!show && <div className="home__custom-link-button-wrapper">
                                <button className="btn" onClick={() => setShow(true)}>لینک دلخواه</button>
                                <span><FaAngleDoubleDown className="mt-2" style={{fontSize:"2.8rem"}}/></span>
                            </div>}
                        </Form>
                    )}
                </Formik>
