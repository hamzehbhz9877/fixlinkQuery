...
const SubmitForgetPassword = () => {
    ...
    const submit = useMutationQuery<null, submitPassword & {token:string}>(submitForgetPassword)

    const handleSubmit = async (values: submitPassword, actions: FormikHelpers<submitPassword>) => {

        actions.resetForm();
        submit.restQuery.mutate({...values, token}, {
            onSuccess: () => {
