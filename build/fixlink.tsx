...
import {adminPath, mainPath, userPath} from "Routes/premissions/route/privateRoles";
            ...
            <Route path={"/account/adminPanel"}>
                <Auth>
                    <PanelsLayout items={AdminItems}>
                        <Switch>
                            {getAllowedRoutes(adminPath).map(({route, path, component, children, ...rest}) =>
                                <Route
                                    {...rest}
                                    key={path}
                                    path={path}
                                >
                                    {
                                        children?.map(({path, ...rest}: any) => <Route {...rest}
                                                                                      key={path}
                                                                                      path={path}/>)
                                    }
