...
interface Props {
    items: { to: string, fa: IconType, text: string,permission?:Array<string> }[],
    open: boolean,
    close: () => void,
    name: string
}
                ...
                <div className="navigation-menu-body">
                    <ul className="navigationApps">
                        {getAllowedRoutes(items).map((item:any, index:number) => {
                                return <li key={index} onClick={close}>
                                    <NavLink to={url + item.to} className="nav-link"
                                             activeClassName="active-side-bar" exact>
                                        <item.fa/>
                                        <span>{item.text}</span>
                                    </NavLink>
                                </li>
                        ...
                        {
                            name === "panel" && <li>
                                <NavLink to="/account/logout" className="nav-link" activeClassName="active-side-bar"
                                         exact>
                                    <FaSignOutAlt/> خروج
                                </NavLink>
                            </li>
